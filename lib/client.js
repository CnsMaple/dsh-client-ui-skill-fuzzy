window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = ".FHNNmq_card{flex-direction:column;display:flex}.FHNNmq_row{align-items:center;min-width:0;height:24px;display:flex;position:relative;overflow:hidden}.FHNNmq_row[data-expandable]{cursor:pointer}.FHNNmq_card[data-state=running] .FHNNmq_row:after{content:\"\";background:linear-gradient(90deg, transparent 0%, color-mix(in srgb, var(--dsw-alias-bg-base) 60%, transparent) 55%, transparent 100%);pointer-events:none;width:300px;animation:2.6s ease-out infinite FHNNmq_dsh-skill-row-sweep;position:absolute;inset:0 auto 0 0}@keyframes FHNNmq_dsh-skill-row-sweep{0%{left:-300px}90%,to{left:100%}}.FHNNmq_leading{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);flex:none;justify-content:center;align-items:center;margin-right:6px;display:inline-flex;position:relative}.FHNNmq_chevron{color:var(--dsw-alias-label-secondary)}.FHNNmq_iconIdle{opacity:1;transition:opacity .1s;display:inline-flex}.FHNNmq_chevronHover{opacity:0;margin:auto;transition:opacity .1s;position:absolute;inset:0}.FHNNmq_row:hover .FHNNmq_iconIdle{opacity:0}.FHNNmq_row:hover .FHNNmq_chevronHover{opacity:1}.FHNNmq_title{color:var(--dsw-alias-label-secondary);flex:none;font-size:14px;line-height:24px}.FHNNmq_separator{background:var(--dsw-alias-label-caption);border-radius:1px;flex:none;width:2px;height:2px;margin:0 8px}.FHNNmq_summary{text-overflow:ellipsis;white-space:nowrap;min-width:0;color:var(--dsw-alias-label-tertiary);flex:auto;font-size:14px;line-height:24px;overflow:hidden}.FHNNmq_errorSummary{color:var(--dsw-alias-state-error-primary)}.FHNNmq_bodyWrap{flex-direction:column;display:flex}.FHNNmq_instructionsCard{border:.5px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-markdown-code-block);border-radius:12px;flex-direction:column;max-height:260px;margin:4px 0 4px 4px;display:flex;overflow:hidden}.FHNNmq_instructionsHeader{border-bottom:.5px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-markdown-code-block-banner);color:var(--dsw-alias-label-caption);text-transform:uppercase;letter-spacing:.04em;flex:none;padding:8px 12px;font-size:11px;font-weight:500;line-height:16px}.FHNNmq_instructions{white-space:pre-wrap;overflow-wrap:anywhere;min-height:0;font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-secondary);margin:0;padding:10px 12px 12px;overflow:auto}.FHNNmq_instructions[data-error]{color:var(--dsw-alias-state-error-primary)}.FHNNmq_instructions::-webkit-scrollbar-thumb{background-clip:padding-box;border:2px solid #0000;border-radius:6px}.FHNNmq_instructions::-webkit-scrollbar-track{margin:6px 0}.FHNNmq_inspectButton{border:.5px solid var(--dsw-alias-border-l4);corner-shape:round;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-secondary);cursor:pointer;opacity:0;border-radius:999px;align-self:flex-start;align-items:center;gap:4px;margin:4px 0 2px 4px;padding:2px 8px;font-size:11px;line-height:16px;transition:opacity .1s;display:inline-flex}.FHNNmq_card:hover .FHNNmq_inspectButton,.FHNNmq_inspectButton:focus-visible{opacity:1}.FHNNmq_inspectButton:hover{background:var(--dsw-alias-interactive-bg-hover-solid);color:var(--dsw-alias-label-primary)}.FHNNmq_visuallyHidden{clip:rect(0 0 0 0);white-space:nowrap;width:1px;height:1px;position:absolute;overflow:hidden}@media (prefers-reduced-motion:reduce){.FHNNmq_card[data-state=running] .FHNNmq_row:after{animation:none;display:none}.FHNNmq_iconIdle,.FHNNmq_chevronHover,.FHNNmq_inspectButton{transition:none}}";
		const tagId = "dsh-client-ui-skill-fuzzy/SkillRow.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-client-ui-skill-fuzzy";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var SkillRow_module_css_default = {
			"bodyWrap": "FHNNmq_bodyWrap",
			"card": "FHNNmq_card",
			"chevron": "FHNNmq_chevron",
			"chevronHover": "FHNNmq_chevronHover",
			"dsh-skill-row-sweep": "FHNNmq_dsh-skill-row-sweep",
			"errorSummary": "FHNNmq_errorSummary",
			"iconIdle": "FHNNmq_iconIdle",
			"inspectButton": "FHNNmq_inspectButton",
			"instructions": "FHNNmq_instructions",
			"instructionsCard": "FHNNmq_instructionsCard",
			"instructionsHeader": "FHNNmq_instructionsHeader",
			"leading": "FHNNmq_leading",
			"row": "FHNNmq_row",
			"separator": "FHNNmq_separator",
			"summary": "FHNNmq_summary",
			"title": "FHNNmq_title",
			"visuallyHidden": "FHNNmq_visuallyHidden"
		};
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			/**
			* Score the strongest ordered-subsequence alignment in O(name x query).
			* Boundary and adjacent matches earn weight; skipped and leading
			* characters cost weight.
			*/
			...
			const source = {
				...
				async candidates(session, { query, signal }) {
					...
					return fuzzyCandidates(skills, query).map((skill) => ({
						name: skill.name,
